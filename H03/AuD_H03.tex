...
\begin{document}
	...
	\section{H3}
		...
		\subsection{c)}
			...
			\underline{Laufzeit:} $ (n-1) \cdot n \in \text{O}(n^{2}) $ \\
			\\
			...
			\\
			\textbf{Fortsetzung:} \\
			5 Verscheidenen Fälle sind zu betrachten:
			\begin{itemize}
				\setlength\itemsep{3pt}
				\item Wenn p das letzte Element ist, besteht M schon aus den größten Elementen.
				\item Wenn q das letzte Element ist, dann ist p ein maximaler Punkt und wir können nach diesem weitersuchen.
				\item Wenn p gleich q ist, dann vergleichen wir direkt p mit dem Element nach q.
				\item Wenn der Punkt p kleiner dem Punkt q ist, dann ist der Punkt p definitiv nicht der maximale Punkt sein kann und können q mit allen anderen Punkten vergleichen.
				\item In allen anderen Fällen können wir einfach das nächsten Punkte vergleichen.
			\end{itemize}
		\end{minipage}
		...
		\begin{flushleft}
			\textbf{Terminierung:} \\
			Compare terminiert, wenn die Bedingung in Zeile 1 zutrifft und somit M zurückgegeben wird. Dies wird erst erreicht, wenn p von 0 bis zum letzten Element gelaufen ist.
			p wird immer dann erhöht, wenn es mit jedem anderen Element verglichen wurde oder sicher ist, dass es kein maximaler Punkt ist.
		\end{flushleft}
